...
from cv_profile import read_cv_header_profile, read_cv_name
...
def maintain_llms_identity(text: str, name: str, roles: list[str], affiliation: str) -> str:
...
def validate_identity_independence() -> None:
    ...
    maintained_llms = maintain_llms_identity(
        llms, synthetic_name, synthetic_roles, synthetic_affiliation
    )
    expected_role = "Research Fellow | Computer Vision Researcher @ Example University"
    if maintained_llms.count(synthetic_name) != 2 or old_name in maintained_llms:
...
def main() -> None:
    ...
    LLMS_PATH.write_text(
        maintain_llms_identity(
            LLMS_PATH.read_text(encoding="utf-8"), name, roles, affiliation
        ),
        encoding="utf-8",
    )
