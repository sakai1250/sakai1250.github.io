...
legacy_buttons = (
    ...
    '<button class="header-btn" id="lang-toggle" type="button" aria-label="Switch to English / 英語に切り替え">',
)
current_button = (
    '<button class="header-btn" id="lang-toggle" type="button" '
    'aria-label="英語に切り替え">'
...
legacy_language_bootstrap = """  <!-- Language Initialization: Resolve Saved or Browser Preference Before Paint -->
  <script>
    (function () {
...
if language_bootstrap not in index_text:
    if legacy_language_bootstrap in index_text:
        index_text = index_text.replace(legacy_language_bootstrap, language_bootstrap, 1)
    elif theme_anchor in index_text:
...
if legacy_language_bootstrap in index_text:
    raise SystemExit("Language bootstrap still exposes mixed-language accessible names")
...
legacy_accessible_block = """        if (persist) safeStorageSet('lang', l);
...
if current_block not in main_text:
    if legacy_accessible_block in main_text:
        main_text = main_text.replace(legacy_accessible_block, current_block, 1)
    elif previous_accessible_block in main_text:
...
for legacy in (
    ...
    if legacy in main_text or legacy in index_text:
        raise SystemExit(f"Mixed-language control label remains: {legacy}")
